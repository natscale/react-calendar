...
<Hint type="warning">
  The calendar component uses react hooks so it requires the react version to be at least **16.8.0**. Make sure you have
  a react version that is equal to or greater than that.
</Hint>
